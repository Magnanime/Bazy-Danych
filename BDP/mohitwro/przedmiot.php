...
<body>
	<header>
		...
		if ($ile>=1) {
			for ($i = 1; $i <= $ile; $i++) {
				$row = mysqli_fetch_assoc($rezultat);
				$kat = $row['nazwa'];
				echo "<a href='kate.php?kategoria=".$kat."'><div class='menu'>$kat</div></a>";
			}
		}
		?>
			<a href="kontakt.php"><div class="menu">Kontakt</div></a>
		</nav>
		<a href="cart.php"><div class="basket"><div class="many">
		<?php 
		if((isset($_SESSION['log'])) && ($_SESSION['log']==true)) {
		require_once "config.php";
		$polaczenie = @new mysqli($host,$db_user,$db_password,$db_name);
	...
	<div class="left-side">
		<div class="navigue">
			...
			<?php
				require_once 'config.php';
				$polaczenie = @new mysqli($host,$db_user,$db_password,$db_name);
				$zapytanie = "SELECT * FROM kategorie ORDER BY id_kat ASC LIMIT 6";
				$rezultat = mysqli_query($polaczenie, $zapytanie);
				$ile = mysqli_num_rows($rezultat);
				if ($ile>=1) {
					for ($i = 1; $i <= $ile; $i++) {
						$row = mysqli_fetch_assoc($rezultat);
						$kat = $row['nazwa'];
						echo "<li><a href='kate.php?kategoria=".$kat."'>$kat</a></li>";
					}
				}
			?>
			</ul>
	...
	<div class="containerprzed">
		<div class="przedmiotpic"> <?php echo "<img src='".$obraz."' height='100%' width='100%'/>"?></div>
		<div class="przedmiotmain">
		<?php 
		$text = "<h1 style='height: 50%;'>".$nazwa."</h1><br><h1 style='text-align: right;'>";
		echo $text;
		echo "<a href='add_to_cart.php?id=".$id."&nazwa=".$nazwa."&cena=".$cena."&ilosc=1'><div class='add_koszyk'>Dodaj do koszyka</div></a>";
		printf ("%.2f", $cena);
		$text = " zł</h1>";
		echo $text;
		?></div>
		...
		<div class="produktopis">
			<?php 
			echo "#".$id; 
			echo "<div class='spec'><h2>SPECYFIKACJA</h2></div>";
			if($producent != " ") echo "<div class='spec'><div class='speckol'>Producent</div><div class='speckol'>".$producent."</div><div style='clear:both;'></div></div>";
			$cecha = explode(": ", $cecha);
			echo "<div class='spec'><div class='speckol'>".$cecha[0]."</div><div class='speckol'>".$cecha[1]."</div><div style='clear:both;'></div></div>";
			echo "<div class='spec'><div class='speckol'>Dostępna ilość</div><div class='speckol'>".$dost." sztuk</div><div style='clear:both;'></div></div>";
			echo "<h2>OPIS</h2><p>".$opis."</p><br><br>"; 
			?>
		</div>
