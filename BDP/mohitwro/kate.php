...
<body>
	...
	require_once 'config.php';
	$polaczenie = @new mysqli($host,$db_user,$db_password,$db_name);
	$zapytanie = "SELECT * FROM towar WHERE kategoria = '$kategoria' AND dostepnosc > 0 ORDER BY id DESC";
	$rezultat = mysqli_query($polaczenie, $zapytanie);
	$ile = mysqli_num_rows($rezultat);
	if ($ile>0) {
		for ($i = 1; $i <= $ile; $i++) {
			...
			$cena = $row['cena'];
			$obraz = $row['obraz'];
			$text = "<a href='przedmiot.php?id=".$id."'><div class='cell'><div class='pic'><img src='".$obraz."' height='100%' width='100%'/></div><div class='name'>".$nazwa."</div><div class='cena'>";
			echo $text;
			printf ("%.2f", $cena);
			$text = " zł</div><div class='index'>#".$id."</div><div style='clear:both;'></div></div></a>";
			echo $text;
		}
	}
	?>
